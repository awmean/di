...
class OrderBase(BaseModel):
    customer_name: str = Field(
        ..., min_length=1, max_length=100, description="Customer name"
    )
    customer_phone: str = Field(
        ..., min_length=1, max_length=20, description="Customer phone"
    )
    comment: Optional[str] = Field(None, description="Order comment")
    status: str = Field(
        default="new",
        pattern="^(new|processing|completed|cancelled)$",
...
class OrderUpdate(BaseModel):
    ...
    status: Optional[str] = Field(
        None, pattern="^(new|processing|completed|cancelled)$"
    )
